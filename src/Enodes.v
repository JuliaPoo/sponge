...
Module PTree.
    ...
    *)
    ...
    Fixpoint tree_fold_preorder' (m: tree' A) (acc : B) : B :=
      match m with
      | Node001 r => tree_fold_preorder' r acc
      | Node010 x => node acc x 
      | Node011 x r => tree_fold_preorder' r (node acc x)  
      | Node100 l => tree_fold_preorder' l acc
      | Node101 l r => tree_fold_preorder' r (tree_fold_preorder' l acc) 
      | Node110 l x => tree_fold_preorder' l (node acc x) 
      | Node111 l x r => tree_fold_preorder' r (tree_fold_preorder' l (node acc x))
      end.

    Definition tree_fold_preorder (m: tree A) (acc : B): B :=
      match m with
      | Empty => acc 
      | Nodes m' => tree_fold_preorder' m' acc
      end.

    Lemma unroll_tree_fold_preorder: forall l o r acc,
      tree_fold_preorder (Node l o r) acc = tree_fold_preorder r (tree_fold_preorder l (match o with 
                                                              | Some o => node acc o
                                                              | None => acc 
                                                            end)).
    Proof.
    ...
    Fixpoint tree_fold_inorder' (m: tree' A) (acc : B) : B :=
      match m with
      | Node001 r => tree_fold_inorder' r acc
      | Node010 x => node acc x 
      | Node011 x r => tree_fold_inorder' r (node acc x)
      | Node100 l => tree_fold_inorder' l acc
      | Node101 l r => tree_fold_inorder' r (tree_fold_inorder' l acc) 
      | Node110 l x => node (tree_fold_inorder' l acc) x 
      | Node111 l x r => tree_fold_inorder' r (node (tree_fold_inorder' l acc) x)
      end.

    Definition tree_fold_inorder (m: tree A) (acc : B): B :=
      match m with
      | Empty => acc 
      | Nodes m' => tree_fold_inorder' m' acc
      end.

    Lemma unroll_tree_fold_inorder: forall l o r acc,
      tree_fold_inorder (Node l o r) acc = tree_fold_inorder r (match o with 
                                                              | Some o =>  node (tree_fold_inorder l acc) o
                                                              | None => (tree_fold_inorder l acc) 
                                                            end).
    Proof.
    ...
    - rewrite unroll_tree_rec by auto. 
      ...
      + simpl. rewrite gmap_filter. destruct (get i (Node l o r)); auto.
    ...
    Definition merge_l {A: Type} (m1 m2: tree A) : tree A :=
     PTree.combine
            (fun lhs rhs =>
              match lhs, rhs with
              | Some l, _ => Some l
    ...
    Definition merge_r {A: Type} (m1 m2: tree A) : tree A :=
     PTree.combine
            (fun lhs rhs =>
              match lhs, rhs with
              | _, Some r => Some r 
            ...
            m1 m2.
    Definition merge_with {A: Type} (m1 m2: tree A) (merge_fn : A -> A -> A): tree A :=
    PTree.combine
            (fun lhs rhs =>
              match lhs, rhs with
              | Some l, Some r =>
